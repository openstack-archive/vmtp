...
class KBScheduler(object):
    ...
    def polling_vms(self, timeout, retry_func=None, polling_interval=None):
        '''
    ...
    def wait_for_vm_up(self, timeout=120):
        cnt_succ = self.polling_vms(timeout, retry_func="redis_report_hello()")[0]
    ...
    def check_http_server_up(self, timeout=60):
        cnt_succ = self.polling_vms(timeout, retry_func="check_http_service()")[0]
        if cnt_succ != len(self.client_list):
    ...
    def run_http_test(self):
        ...
        cnt_succ = self.polling_vms(timeout, polling_interval=120)[0]
        if cnt_succ != len(self.client_list):
