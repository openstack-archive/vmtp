...
Provider networks are created by network administrators, which specifys the details of how network is physically realized, and usually match some existing networks in the data center. In general, provider networks only handle layer-2 connectivity for instances, and lack the concept of fixed and floating IP addresses.

VMTP supports to run on a provider network by either supplying the provider network via either CLI or config file. Just supply the provider network name with **--reuse_network_name <network_name>**, and VMTP will perform the L2-only throughput tests on that provider network.
