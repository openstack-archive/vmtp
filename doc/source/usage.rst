...
Create one configuration file for your specific cloud and use the *-c* option to pass that file name to VMTP. Parameters that you are most certainly required to change are:
    ...
    **availability_zone**: The name must be set to the name of an availability zone defined in openstack, i.e. as shown from "nova availability-zone-list" or use Horizon to get the list of availability zones. (also specific to each cloud)
